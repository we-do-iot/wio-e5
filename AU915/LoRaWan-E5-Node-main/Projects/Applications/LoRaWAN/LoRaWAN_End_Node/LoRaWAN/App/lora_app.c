...
void LoRaWAN_Init(void)
{
  ...
  LoraInfo_Init();
	if (ACTIVE_REGION == LORAMAC_REGION_AU915){
		// Deshabilitar todos los canales excepto 8–15 y el canal 66
		  for (uint8_t i = 0; i < 72; i++)
		  {
			  if ((i < 8) || (i > 15 && i != 66))
			  {
				  ChannelRemoveParams_t chanRemove;
				  chanRemove.ChannelId = i;
				  RegionChannelsRemove(ACTIVE_REGION, &chanRemove);
			  }
		  }
	}
